...
The diagram above is a subtle description of what our project is designed to do. Tasks refer to the task of vm creation, similar to the `Instances` in P1. Initially, tasks are generated randomly by our script and are hosted in the private cloud. New tasks keep getting generated and after the private cloud reaches the 80% threshold, any new task that is generated will be scheduled to the public cloud. We only optimize based on cpu usage based on our analysis in [Expected resource requirements](#expected-resource-requirements).

The script also deletes the tasks randomly. Hence it is possible that the private cloud cpu usage goes below 50%. In that casewe are moving some of the tasks from public cloud back to the private cloud to ensure efficient usage of private cloud. The next step is to consolidate the remaining tasks in the public cloud and stop any machines that aren't running any tasks.

Given below are some of our design choices and assumptions which will explain the project in more detail.

#### Full Ownership
We are assuming that we own both the private and the public cloud so that, we are responsible for taking care of all the load-balancing and defragmentation ourselves. If we use AWS, it'll do all the optimizations in public cloud automatically and that would limit the scope of our project. 

We are generating tasks based on probabilities and fixed contants mentioned in `constants.py`. We are using some constraints such as probability of creating a new task, probability of deleting an existing task, max and expected resource consumption of a task etc.

#### Homogeneous Cloud
One of the important choices we made is that, we are running a homogeneous datacenter i.e. We have all the software stack we need and all the machines are homogeneous. That way it'll be easier to monitor the usage of each of the machines and prevent fragmentation when moving the tasks from one machine to the other, from private to public cloud or within the public cloud.
...
Consider a simple scenario where we have two machines with 4 vcpus each. Let's say two tasks which require 2 vcpus were created. If we randomly assign one task to each machine and a third task which needs 3 vcpus gets generated, we need to start a third machine to host the task. Instead if we add the second task to the first machine itself, using our Heap machanism, we don't need to start a third machine for hosting the third task. This was just a simplified example to better explain the need for defragmentation.

Note that computing the optimal solution is an NP hard problem, and this is only an approximate solution. Also, we are using only vcpus to sort the machines. However, is sufficient for this project based on the aforementioned assumptions and analysis.
...
* Gained knowledge on how AWS and Azure manage load-balancing in their public cloud
...
We have tried to make the project robust and we believe that it can be extended as a research project in the future as it has practical applications. We are thankful to the instructors for letting us choose our own project and for the continous feedback and guidance.
